...
class CmcOHLCVStep(BaseStep):
    def __init__(self, config, step_config, symbol=None, interval=None, request_wrapper=None):
        ...
        self.name = "CMC"
        self.sub_name = f"OHLCV_{symbol.symbol}_interval"
    ...
    def _get_plan_start_date(self):
        if self.interval == "daily":
            return datetime.now(timezone.utc).date() + relativedelta(months=-self.step_config.daily_range)
        if self.interval == "hourly":
            return datetime.now(timezone.utc) + relativedelta(months=-self.step_config.hourly_range)
    ...
    def _get_data(self):
        ...
        self.counter += 1
        data = self.request_wrapper.get_data(endpoint=self.endpoint, params=self.params)
    ...
    def process(self):
        ...
        if not self.is_done:
            self.send_log(phase=self.sub_name, progress_text=f"{self.params['time_start']} [{self.counter:>3}]")
            return
