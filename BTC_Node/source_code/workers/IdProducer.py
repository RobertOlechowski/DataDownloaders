...
class IdProducer(BaseWorker):
    def __init__(self, index, stop_event):
        ...
        self.session = sessionmaker(bind=self.engine)() if self.engine is not None else None
    ...
    def get_top_block_height(self):
        ...
        if self.config.mode.insert_db:
            top_heights = self.session.query(BtcBlockDAO.height).order_by(BtcBlockDAO.height.desc()).limit(_test_range).all()
